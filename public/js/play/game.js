...
let hasCardMoved = false; //shows if an active card has been moved
...
document.addEventListener('mousemove', e => { //on mouse move
  ...
  if (activeComponent) {
    ...
    if (compType === "card") hasCardMoved = true; //if active component is card, show that card has moved
...
document.addEventListener('mouseup', e => {
  ...
  hasCardMoved = false; //reset if card has moved status
}); 
...
const backImgPath = '/images/play/cards/back.svg'; //get img path for back of card
const cards = document.querySelector('#cards'); //get card parent container

function createCard(imgPath, x, y, id, flippedOver) { //creates card

  let card = document.createElement('img'); //create img element
  card.originalSrc = imgPath; //set source of card when not flipped over
  card.src = imgPath; //set image path

  card.setAttribute("draggable", false); //make card undraggable

  card.classList.add("card"); //add card class to element
  card.id = id; //set card id

  //position card (add offset since the image is center aligned)
  card.style.left = `${x}px`;
  card.style.top = `${y}px`; 

  cards.appendChild(card); //append element

  flipCard(card.id, flippedOver); //flip card accordingly

  card.addEventListener('mousedown', e => { //on card select
    activeComponent = card; //set active component to card
    cards.appendChild(card); //move card to top of list
  });

  card.addEventListener('mouseup', e => { //on card mouseup
    if (!hasCardMoved) { //if card has not moved, flip it over
      flipCard(card.id); //flip card
      socket.emit("flipCard", { gameId: gameId, id: card.id, flippedOver: card.flippedOver }); //broadcast to other players to flip card
    }
  });

  return { x: x, y: y, id: card.id, elm: card }; //return info about card
}

function flipCard(id, flippedOver) { //on card flip (only for client side)
  const card = document.querySelector(`#${id}`); //get card element

  if (flippedOver === undefined) card.flippedOver = !card.flippedOver; //negate card flipped over status if no flipped specified 
  else card.flippedOver = flippedOver; //set flippedOver status if applicable

  if (card.flippedOver) card.src = '/images/play/cards/back.svg'; //if card is now flipped over show back of card
  else card.src = card.originalSrc;  //else show front of card
  
}

socket.emit('getCards', gameId, response => {
  const cards = response; //get cards from response

  cards.forEach(card => { //iterate through cards
    createCard(card.imgPath, //create card
        card.pcX * gameboard.width, card.pcY * gameboard.height,
        card.id, card.flippedOver);
  });

});

socket.on('flipCard', info => { flipCard(info.id, info.flippedOver) }); //flip card